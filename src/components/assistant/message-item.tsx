...
export function MessageItem({ role, text }: Message) {
  const isAssistant = role === "assistant"
  return (
    <div className={cn("flex items-start gap-3", isAssistant ? "justify-start" : "justify-end")} role="listitem">
      {isAssistant && (
      ...
      >
        {isAssistant ? <MarkdownRenderer content={text} /> : <div className="text-white">{text}</div>}
      </Card>
