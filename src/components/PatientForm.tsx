...
function validate(values: PatientFormValues): Errors {
  ...
  else if (!zipRegex.test(values.zip.trim())) errs.zip = "Enter a valid US ZIP (12345 or 12345-6789)."
  return errs
}

export const PatientForm: React.FC<Props> = ({ onSubmit, initialValues }) => {
  const [values, setValues] = useState<PatientFormValues>({
    firstName: initialValues?.firstName ?? "",
    lastName: initialValues?.lastName ?? "",
    dob: initialValues?.dob ?? "",
    gender: (initialValues?.gender as PatientFormValues["gender"]) ?? "",
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setTouched({ firstName: true, lastName: true, dob: true, gender: true, zip: true })
        ...
        <div className="grid">
          ...
          <div className="field">
            ...
            {touched.dob && errs.dob && (
              <p id="dob-error" className="error">
                {errs.dob}
              </p>
            )}
          </div>
          ...
          <div className="field">
            ...
            {touched.gender && errs.gender && (
              <p id="gender-error" className="error">
                {errs.gender}
              </p>
            )}
          </div>
          ...
          <div className="field field--full">
            ...
            {touched.zip && errs.zip && (
              <p id="zip-error" className="error">
                {errs.zip}
              </p>
            )}
          </div>
